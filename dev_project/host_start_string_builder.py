...
class StartStringBuilder():
    ...
    def get_start_string(self):
        ...
        start_python_command = f"""python3 -u -m debugpy --listen 0.0.0.0:{DEBUGGER_DEFAULT_PORT} {self.config["docker_odoo_dir"]}/odoo-bin -c {self.config["docker_project_dir"]}/odoo.conf --limit-time-real 99999"""
        ...
        if start_pre_commit:
            start_string = f"""/bin/bash -c 'cd {self.config["docker_odoo_project_dir_path"]} && ls && git config --global --add safe.directory git config --global --add safe.directory /workspaces/core && pre-commit run --all-files'"""
            return start_string
