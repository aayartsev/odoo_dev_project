...
PIP_INSTALL_COMMAND="cd ${DOCKER_PROJECT_DIR} && python3 -m venv ${DOCKER_VENV_DIR} && . ${DOCKER_VENV_DIR}/bin/activate && wget -O odoo_requirements.txt https://raw.githubusercontent.com/odoo/odoo/$ODOO_VERSION/requirements.txt && python3 -m pip install -r odoo_requirements.txt && python3 -m pip uninstall -y -r requirements_uninstall.txt && python3 -m pip install -r ${DOCKER_PROJECT_DIR}/requirements.txt"
